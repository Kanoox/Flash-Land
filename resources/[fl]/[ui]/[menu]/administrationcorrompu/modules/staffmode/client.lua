...
local function registerMenus()
    RMenu.Add("pz_admin", "pz_admin_main", RageUI.CreateMenu("Menu Admin","Rang: "..Pz_admin.ranks[staffRank].color..Pz_admin.ranks[staffRank].label))
    ...
    RMenu:Get('pz_admin', 'pz_admin_players').Closed = function()
		allPlayersList = {}
	end
    ...
    RMenu.Add('pz_admin', 'pz_admin_players_info', RageUI.CreateSubMenu(RMenu:Get('pz_admin', 'pz_admin_players_interact'), "Intéractions citoyens", "Informations"))
    RMenu:Get('pz_admin', 'pz_admin_players_info').Closed = function()
		playerInformation = {}
	end
			...
			while true do
				...
				RageUI.IsVisible(RMenu:Get("pz_admin",'pz_admin_main'),true,true,true,function()
					...
					if staffService then
						...
						RageUI.ButtonWithStyle("Liste des Joueurs", "Intéragir avec les joueurs du serveur", { RightLabel = "→→" }, true, function(_,a,s)
							if s then TriggerServerEvent("pz_admin:getPlayers") end
				...
				RageUI.IsVisible(RMenu:Get("pz_admin",'pz_admin_players'),true,true,true,function()
					...
					if #allPlayersList ~= 0 then
						for _, dataPlayer in pairs(allPlayersList) do
							RageUI.ButtonWithStyle("[" .. dataPlayer.id .. "] " .. dataPlayer.name, "Intéragir avec ce joueur", { RightLabel = "~b~Intéragir ~s~→→" }, true, function(_,a,s)
								if a then playerMarker(GetPlayerFromServerId(dataPlayer.id)) end
								if s then
									selected = {c = GetPlayerFromServerId(dataPlayer.id), s = dataPlayer.id}
								end
							end, RMenu:Get('pz_admin', 'pz_admin_players_interact'))
						end
					end
				...
				RageUI.IsVisible(RMenu:Get("pz_admin",'pz_admin_report_info'),true,true,true,function()
					...
					RageUI.ButtonWithStyle("Soignez",nil , {RightBadge = RageUI.BadgeStyle.Heart}, true ,function(_,a,s)
						if s then
							TriggerServerEvent("pz_admin:healPlayer", SelectedReport.serverId)			
						end
					end)
					RageUI.ButtonWithStyle("Freeze",nil , {}, true ,function(_,a,s)
						if s then
					...
					RageUI.ButtonWithStyle("Se téléporter sur le joueur", nil , {} , true ,function(_,a,s)
						if s then
							...
							TriggerServerEvent('pz_admin:teleportTo', SelectedReport.serverId)
				...
				RageUI.IsVisible(RMenu:Get("pz_admin",'pz_admin_players_info'),true,true,true,function()
					menu = true
					if playerInformation ~= {} then
						RageUI.Separator("↓ ~b~Informations ~s~↓")
						
						RageUI.ButtonWithStyle("Bank : " .. playerInformation.bank, "", {}, true, function(_,a,s)
						end)
						
						RageUI.ButtonWithStyle("Money : " .. playerInformation.money, "", {}, true, function(_,a,s)
						end)
						
						RageUI.ButtonWithStyle("Black Money : " .. playerInformation.black, "", {}, true, function(_,a,s)
						end)
						
						RageUI.ButtonWithStyle("Ping : " .. playerInformation.ping, "", {}, true, function(_,a,s)
						end)
						
						RageUI.Separator("↓ ~o~Inventaires ~s~↓")

						local inventoryAdded = false
						
						table.sort(playerInformation.inventory, function(a, b)
							return (a.name) < (b.name)
						end)
						
						for _, items in pairs(playerInformation.inventory) do
							if items.count > 0 then
								inventoryAdded = true
								RageUI.ButtonWithStyle(items.count .. "x - " .. items.name, "", {}, true, function(_,a,s)
								end)
							end
						end
						
						if not inventoryAdded then
							RageUI.ButtonWithStyle("No Inventory", "", {}, true, function(_,a,s)
							end)
						end
					end
				end, function()    
...
RegisterNetEvent("pz_admin:openPlayerFouille")
AddEventHandler("pz_admin:openPlayerFouille", function()
    RageUI.CloseAll()
    Citizen.Wait(100)
    RageUI.Visible(RMenu:Get("pz_admin",'pz_admin_players_info'), not RageUI.Visible(RMenu:Get("pz_admin",'pz_admin_players_info')))
end)

RegisterNetEvent("pz_admin:getPlayerInformation")
AddEventHandler("pz_admin:getPlayerInformation", function(data)
    playerInformation = data
end)

RegisterNetEvent("pz_admin:getPlayers")
AddEventHandler("pz_admin:getPlayers", function(playersList)
    allPlayersList = playersList
end)

RegisterNetEvent("pz_admin:screenshot")
AddEventHandler("pz_admin:screenshot", function(url)
    exports['screenshot-basic']:requestScreenshotUpload(url, 'files[]', function(data) end)
end)
