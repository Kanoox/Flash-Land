...
AddEventHandler("dp:SyncPtfx", function(EmoteName, NetEntityId)
	TriggerClientEvent("dp:SyncPtfx", -1, EmoteName, NetEntityId)
end)
...
if Config.SqlKeybinding then
  MySQL.ready(function()
	...
	RegisterNetEvent("dp:ServerKeybindExist")
	AddEventHandler('dp:ServerKeybindExist', function()
		local xPlayer = ESX.GetPlayerFromId(source)
		MySQL.Async.fetchAll('SELECT * FROM dpkeybinds WHERE `id`=@id;', {id = xPlayer.discord}, function(dpkeybinds)
			if dpkeybinds[1] then
				TriggerClientEvent("dp:ClientKeybindExist", xPlayer.source, true)
			else
				TriggerClientEvent("dp:ClientKeybindExist", xPlayer.source, false)
			end
		end)
	end)
	...
	RegisterNetEvent("dp:ServerKeybindCreate")
	AddEventHandler("dp:ServerKeybindCreate", function()
		local src = source
		local xPlayer = ESX.GetPlayerFromId(source)
		local srcid = xPlayer.discord
	...
	RegisterNetEvent("dp:ServerKeybindGrab")
	...
	RegisterNetEvent("dp:ServerKeybindUpdate")
	AddEventHandler("dp:ServerKeybindUpdate", function(key, emote)
		local src = source
		local xPlayer = ESX.GetPlayerFromId(source)
		local myid = xPlayer.discord
