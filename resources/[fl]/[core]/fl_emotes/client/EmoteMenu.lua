...
mainMenu = NativeUI.CreateMenu("FreeLife", "", menuPosition["x"], menuPosition["y"], Menuthing, Menuthing)
...
function AddEmoteMenu(menu)
    ...
    for a,b in pairsByKeys(DP.Emotes) do
      if DP.Emotes[a].Menu then
        if not DP.AllMenu[DP.Emotes[a].Menu].SubMenu then
          local sMenu = _menuPool:AddSubMenu(submenu, DP.AllMenu[DP.Emotes[a].Menu].Label, "", "", Menuthing, Menuthing)
          DP.AllMenu[DP.Emotes[a].Menu].SubMenu = sMenu
          DP.AllMenu[DP.Emotes[a].Menu].EmoteTable = {}
          table.insert(EmoteTable, {menu = DP.Emotes[a].Menu})
        end
      end
    end
    ...
    for a,b in pairsByKeys(DP.Emotes) do
      ...
      if DP.Emotes[a].Menu then
        DP.AllMenu[DP.Emotes[a].Menu].SubMenu:AddItem(emoteitem)
        table.insert(DP.AllMenu[DP.Emotes[a].Menu].EmoteTable, a)
      else
    ...
    if not Config.SqlKeybinding then
      favmenu.OnItemSelect = function(sender, item, index)
        if FavEmoteTable[index] == Config.Languages[lang]['rfavorite'] then
          FavoriteEmote = ""
          ESX.ShowNotification(Config.Languages[lang]['rfavorite'], 2000)
        return end
        if Config.FavKeybindEnabled then
          FavoriteEmote = FavEmoteTable[index]
          ESX.ShowNotification("~o~"..firstToUpper(FavoriteEmote)..Config.Languages[lang]['newsetemote'])
        end
      end
    end
    ...
    for i,MenuInfo in pairs(DP.AllMenu) do
      MenuInfo.SubMenu.OnItemSelect = function(sender, item, index)
        EmoteMenuStart(MenuInfo.EmoteTable[index], "emotes")
      end
    end

    submenu.OnItemSelect = function(sender, item, index)
     if EmoteTable[index] ~= Config.Languages[lang]['favoriteemotes'] then
      if type(EmoteTable[index]) == 'table' then -- This is a menu
      else
...
function AddFaceMenu(menu)
    ...
    submenu.OnItemSelect = function(sender, item, index)
      if item ~= facereset then
        SetResourceKvp('dpemotes:lastMood', FaceTable[index])
        EmoteMenuStart(FaceTable[index], "expression")
      else
        DeleteResourceKvp('dpemotes:lastMood')
