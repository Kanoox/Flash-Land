...
RegisterCommand('+fl_emotes_bind4', function()
    if IsPedSittingInAnyVehicle(PlayerPedId()) then return end
    if emob1 ~= "" then EmoteCommandStart(nil,{emob1, 0}) end
end, false)

RegisterCommand('+fl_emotes_bind5', function()
    if IsPedSittingInAnyVehicle(PlayerPedId()) then return end
    if emob2 ~= "" then EmoteCommandStart(nil,{emob2, 0}) end
end, false)

RegisterCommand('+fl_emotes_bind6', function()
    if IsPedSittingInAnyVehicle(PlayerPedId()) then return end
    if emob3 ~= "" then EmoteCommandStart(nil,{emob3, 0}) end
end, false)

RegisterCommand('+fl_emotes_bind7', function()
    if IsPedSittingInAnyVehicle(PlayerPedId()) then return end
    if emob4 ~= "" then EmoteCommandStart(nil,{emob4, 0}) end
end, false)

RegisterCommand('+fl_emotes_bind8', function()
    if IsPedSittingInAnyVehicle(PlayerPedId()) then return end
    if emob5 ~= "" then EmoteCommandStart(nil,{emob5, 0}) end
end, false)

RegisterCommand('+fl_emotes_bind9', function()
    if IsPedSittingInAnyVehicle(PlayerPedId()) then return end
    if emob6 ~= "" then EmoteCommandStart(nil,{emob6, 0}) end
end, false)

RegisterCommand('+fl_emotes_handsup', function(source, args, raw)
    if not IsControlPressed(0, 61) or IsControlPressed(0, 19) then return end
    if IsInAnimation then EmoteCancel() return end
    Citizen.Wait(200)
    EmoteCommandStart(source, {'handsup'})
end, false)

RegisterCommand('-fl_emotes_handsup', function() end, false)

RegisterCommand('-fl_emotes_bind4', function() end, false)
RegisterCommand('-fl_emotes_bind5', function() end, false)
RegisterCommand('-fl_emotes_bind6', function() end, false)
RegisterCommand('-fl_emotes_bind7', function() end, false)
RegisterCommand('-fl_emotes_bind8', function() end, false)
RegisterCommand('-fl_emotes_bind9', function() end, false)
