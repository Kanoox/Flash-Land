					...
					if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
						...
						if IsControlJustPressed(1, 38) then
							...
							if driverServer == 0 then
								...
								TriggerServerEvent("fl_core:TargetClient", driverServer, closestTire.tireIndex)
							end
							Citizen.Wait((animDuration / 2) * 1000)
...
RegisterNetEvent("fl_core:SlashClientTire")
AddEventHandler("fl_core:SlashClientTire", function(tireIndex)
	TriggerEvent("chatMessage", "^1le joueur essaie de crever votre pneu")
	local player = PlayerId()
	local plyPed = GetPlayerPed(player)
