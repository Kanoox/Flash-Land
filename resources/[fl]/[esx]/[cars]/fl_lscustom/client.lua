...
Citizen.CreateThread(function()
	ESX.TriggerServerCallback('fl_lscustom:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)

RegisterNetEvent('fl_lscustom:installMod')
AddEventHandler('fl_lscustom:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('fl_lscustom:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('fl_lscustom:cancelInstallMod')
AddEventHandler('fl_lscustom:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('native', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			if k == data.current.modType or isRimMod then
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('fl_lscustom:installMod')
				else
					...
					if isRimMod then
						...
						if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'bennys' then
						    TriggerServerEvent("fl_lscustom:buyMod", price)
					    elseif ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'mechanic' then
							TriggerServerEvent("fl_lscustom:buyModMechanic", price)
						end
		...
		menu.close()
		TriggerEvent('fl_lscustom:cancelInstallMod')
	...
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'mechanic' then
				for k,v in pairs(Config.ZonesMechanic) do
					if #(coords - vector3(v.Pos.x, v.Pos.y, v.Pos.z)) < v.Size.x and v.Name ~= nil then
						isInLSMarker  = true
						ESX.ShowHelpNotification(v.Hint)
						break
