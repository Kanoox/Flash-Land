Config.Jobs.fisherman = {

	BlipInfos = {
		Sprite = 68,
		Color = 38
	},

	Vehicles = {

		Truck = {
			Spawner = 1,
			Hash = "benson",
			Trailer = "none",
			HasCaution = true
		},

		Boat = {
			Spawner = 2,
			Hash = "tug",
			Trailer = "none",
			HasCaution = false
		}

	},

	Zones = {

		CloakRoom = {
			Pos = {x = 868.39, y = -1639.75, z = 29.36},
			Size = {x = 1.5, y = 1.5, z = 0.6},
			Color = {r = 0, g = 208, b = 255},
			Marker = 27,
			Blip = true,
			Name = _U('fm_fish_locker'),
			Type = "cloakroom",
			Hint = _U('cloak_change'),
			GPS = {x = 880.74, y = -1663.96, z = 29.37}
		},

		FishingSpot = {
			Pos = {x = 4435.21, y = 4829.60, z = 0.50},
            Size = {x = 50.5, y = 50.5, z = 1.0},
			Color = {r = 255, g = 0, b = 0},
			Marker = 1,
			Blip = true,
			Name = _U('fm_fish_area'),
			Type = "work",
			Hint = _U('fm_fish_button'),
			GPS = {x = 3859.43, y = 4448.83, z = 0.35},
			Item = {
				{
					name = _U('fm_fish'),
					db_name = "fish",
					time = 4300,
					max = 100,
					add = 1,
					remove = 1,
					requires = "nothing",
					requires_name = "Nothing",
					drop = 100
				}
			},

		},

		BoatSpawner = {
			Pos = {x = 3867.44, y = 4463.62, z = 1.72},
			Size = {x = 1.5, y = 1.5, z = 0.6},
			Color = {r=0,g=202,b=33},
			Marker = 27,
			Blip = true,
			Name = _U('fm_spawnboat_title'),
			Type = "vehspawner",
			Spawner = 2,
			Hint = _U('fm_spawnboat'),
			Caution = 500,
			GPS = {x = 4435.21, y = 4829.60, z = 0.34}
		},

		BoatSpawnPoint = {
			Pos = {x = 3888.3, y = 4468.09, z = 0.0},
			Size = {x = 1.5, y = 1.5, z = 0.6},
			Marker= -1,
			Blip = false,
			Name = _U('fm_boat_title'),
			Type = "vehspawnpt",
			Spawner = 2,
			GPS = 0,
			Heading = 270.1
		},

		BoatDeletePoint = {
			Pos = {x = 3859.43, y = 4448.83, z = 0.42},
			Size = {x = 1.5, y = 1.5, z = 0.6},
			Color = {r=255,g=0,b=0},
			Marker = 27,
			Blip = false,
			Name = _U('fm_boat_return_title'),
			Type = "vehdelete",
			Hint = _U('fm_boat_return_button'),
			Spawner = 2,
			Caution = 500,
			GPS = {x = -1012.64, y = -1354.62, z = 5.54},
			Teleport = {x = 3867.44, y = 4463.62, z = 1.72}
		},

		VehicleSpawner = {
			Pos = {x = 880.74, y = -1663.96, z = 29.40},
			Size = {x = 1.5, y = 1.5, z = 0.6},
			Color = {r=0,g=202,b=33},
			Marker = 27,
			Blip = false,
			Name = _U('spawn_veh'),
			Type = "vehspawner",
			Spawner = 1,
			Hint = _U('spawn_veh_button'),
			Caution = 500,
			GPS = {x = 3867.44, y = 4463.62, z = 1.72}
		},

		VehicleSpawnPoint = {
			Pos = {x = 859.35, y = -1656.21, z = 29.56},
			Size = {x = 1.5, y = 1.5, z = 0.6},
			Marker = -1,
			Blip = false,
			Name = _U('service_vh'),
			Type = "vehspawnpt",
			Spawner = 1,
			GPS = 0,
			Heading = 70.1
		},

		VehicleDeletePoint = {
			Pos = {x = 863.23, y = -1718.28, z = 28.63},
			Size = {x = 1.5, y = 1.5, z = 0.6},
			Color = {r=255,g=0,b=0},
			Marker = 27,
			Blip = false,
			Name = _U('return_vh'),
			Type = "vehdelete",
			Hint = _U('return_vh_button'),
			Spawner = 1,
			Caution = 500,
			GPS = 0,
		},

		Delivery = {
			Pos = {x = -1012.64, y = -1354.62, z = 4.57},
			Size = {x = 2.5, y = 2.5, z = 0.6},
			Color = {r = 0, g = 208, b = 255},
			Marker = 27,
			Blip = true,
			Name = _U('delivery_point'),
			Type = "delivery",
			Spawner = 2,
			Hint = _U('fm_deliver_fish'),
			GPS = {x = 3867.44, y = 4463.62, z = 1.72},
			Item = {
				{
				name = _U('delivery'),
				time = 4500,
				remove = 1,
				max = 100,
				price = 12,
				requires = "fish",
				requires_name = _U('fm_fish'),
				drop = 100
				}
			}
		}

	}
}
