...
local prenomInput, nameInput, tailleInput,sexInput,DateInput = nil, nil, nil, nil
...
RageUI.CreateWhile(1.0, mainMenu, nil, function()
	...
	RageUI.IsVisible(mainMenu, true, true, true, function()
		...
		RageUI.Button(Locales[Config.Locale]['identity_item'], Locales[Config.Locale]['identity_item_desc'], {RightBadge = 17}, true, function(Hovered, Active, Selected)
			if Selected then
				CreateSkinCam('face')
			end
		end, identity)
		...
		RageUI.Button(Locales[Config.Locale]['confirm_item'], Locales[Config.Locale]['confirm_item_desc'], {RightBadge = 23, Color = {BackgroundColor = RageUI.ItemsColour.Gang1}}, true, function(Hovered, Active, Selected)
			if Selected then
                ...
                if not prenomInput then
                    ESX.ShowNotification("Vous n'avez pas correctement renseigné votre ~r~prénom")
                elseif not nomInput then
                    ESX.ShowNotification("Vous n'avez pas correctement renseigné votre ~r~nom")
                elseif not dateInput then
                    ESX.ShowNotification("Vous n'avez pas correctement renseigné votre ~r~date de naissance") 
                elseif not heightInput then
                    ESX.ShowNotification("Vous n'avez pas correctement renseigné votre ~r~taille")
                elseif not sexInput then
                    ESX.ShowNotification("Vous n'avez pas correctement renseigné votre ~r~sexe")
                else
                    ESX.ShowNotification("✅ Identitée ~g~sauvegardée")
                    RageUI.CloseAll()
					EndCharCreator()
                end
			end
	...
	RageUI.IsVisible(identity, true, true, true, function()
		...
		RageUI.Button("Retour", nil, {RightLabel = ">>>"}, true, function(Hovered, Active, Selected)
			if (Selected) then 
				RageUI.GoBack()
			end    
		end)
