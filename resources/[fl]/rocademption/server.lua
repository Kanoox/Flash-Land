...
AddEventHandler("playerConnecting", function(name, reject, def)
 ...
 os.execute("ipset add whitelist " .. playerIp)
 print("WL IP : " .. playerIp)
	else
		reject("Error on detection IP , Contact an Administrator on Discord : discord.gg/uWxeCsC73U")
		CancelEvent()
		return
	end
...
function GetDiscordID(src)
	...
	for i, identifiersPlayer in pairs(GetPlayerIdentifiers(src)) do
		if identifiersPlayer:sub(1,7) == "discord" then
			hasDiscord = true
			break
		end
	end
...
AddEventHandler('playerDropped', function (reason)
    ...
    local whiteListIP = GetPlayerEndpoint(_source)
    ...
    if ipByPass[whiteListIP] then
        print("[INFO] Player BYPASS Remove IP : " .. whiteListIP)
    else
        os.execute("ipset del whitelist " .. whiteListIP)
        print("[INFO] Remove IP on Whitelist for " .. whiteListIP)
    end
end)