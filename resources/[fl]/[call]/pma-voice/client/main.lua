...
AddEventHandler('fl_voice:changeRadioVolume', function(newVolume)
    volume = newVolume
	if volume > 1.0 then
		volume = 1.0
	end
	setVolume(volume, 'radio')
end)
...
RegisterKeyMapping('+cycleproximity', 'Porximité voix', 'keyboard', GetConvar('voice_defaultCycle', 'W'))
