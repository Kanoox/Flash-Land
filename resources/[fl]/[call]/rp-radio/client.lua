...
function Radio:IncreaseVolume()
	if self.Volume < 1.0 then
		self.Volume = self.Volume + 0.1
		TriggerEvent('fl_voice:changeRadioVolume', self.Volume)
    elseif self.Volume >= 1.0 then
        self.Volume = 1.0
	end
end

function Radio:DecreaseVolume()
	if self.Volume > 0.2 then
		self.Volume = self.Volume - 0.1
		TriggerEvent('fl_voice:changeRadioVolume', self.Volume)
	end
end
	...
	if not isFalling and Radio.Enabled and Radio.Has and not isDead then
        ESX.TriggerServerCallback('rp-radio:getItemAmount', function(quantity)
            if quantity > 0 then
                ...
                ESX.ShowNotification('Vous ~r~n\'avez pas~s~ votre radio sur vous !')
                Radio:Toggle(false)
            end
        end)
    elseif (Radio.Open or Radio.On) and ((not Radio.Enabled) or (not Radio.Has) or isDead) then
...
RegisterKeyMapping('openRadio', 'Ouvrir la radio (policiers only)', 'keyboard', 'F4')
...
Citizen.CreateThread(function()
	...
	TriggerEvent('fl_voice:changeRadioVolume', Radio.Volume)
    ...
    while true do
        ...
        if Radio.Open then
            ...
            AddTextComponentSubstringPlayerName(math.floor(Radio.Volume * 100) .. '%')
            ...
            if not radioConfig.Controls.DecreaseVolume.Pressed then
				if IsControlJustPressed(0, radioConfig.Controls.DecreaseVolume.Key) then
					...
					Citizen.CreateThread(function()
						while IsControlPressed(0, radioConfig.Controls.DecreaseVolume.Key) do
			...
			if not radioConfig.Controls.IncreaseVolume.Pressed then
				if IsControlJustPressed(0, radioConfig.Controls.IncreaseVolume.Key) then
					...
					Citizen.CreateThread(function()
						while IsControlPressed(0, radioConfig.Controls.IncreaseVolume.Key) do
        ...
        if Radio.On then
            ESX.TriggerServerCallback('rp-radio:getItemAmount', function(quantity)
                if quantity == 0 then
                    ...
                    Radio:Remove()
                    exports["pma-voice"]:setVoiceProperty("radioEnabled", false)
                    ESX.ShowNotification('~r~Vous n\'avez plus votre radio sur vous.')
                end
            end)
        end
        Wait(20000)
...
AddEventHandler("esx:onPlayerSpawn", function()
    ...
    if job ~= nil and job == 'police' then
        GivePlayerAccessToFrequencies(1, 2)
        RemovePlayerAccessToFrequency(3, 4)
    elseif job ~= nil and job == 'ambulance' then
        GivePlayerAccessToFrequencies(3, 4)
        RemovePlayerAccessToFrequency(1, 2)
    elseif job ~= nil and job == 'gouv' then
        GivePlayerAccessToFrequencies(1, 2, 3, 4, 5)
    end
end)
