...
const state = {
  ...
  airplane: window.localStorage['airplane_mode'] === 'true',
...
PhoneAPI.setAirplaneMode(state.airplane)
...
const getters = {
  ...
  zoom: ({ zoom }) => zoom,
  isAirplaneMode: ({ airplane }) => airplane,
...
const actions = {
  ...
  setAirplaneMode ({ commit }, value) {
    window.localStorage['airplane_mode'] = value
    PhoneAPI.setAirplaneMode(value)
    commit('SET_AIRPLANE_MODE', value)
  },
  ...
  setMyPhoneNumber ({ commit }, value) {
    commit('SET_MY_PHONE_NUMBER', value)
  },
  closePhone () {
    PhoneAPI.closePhone()
  },
  resetPhone ({ dispatch, getters }) {
    ...
    dispatch('setAirplaneMode', false)
  }
}
...
const mutations = {
  ...
  SET_MY_PHONE_NUMBER (state, myPhoneNumber) {
    const hasSim = myPhoneNumber !== 0 && myPhoneNumber !== '#######'

    const appIndex1 = state.config.apps.findIndex(app => app.routeName === 'appels')
    if (appIndex1 !== -1) {
      Vue.set(state.config.apps[appIndex1], 'enabled', hasSim)
    }

    const appIndex2 = state.config.apps.findIndex(app => app.routeName === 'messages')
    if (appIndex2 !== -1) {
      Vue.set(state.config.apps[appIndex2], 'enabled', hasSim)
    }

    const appIndex3 = state.config.apps.findIndex(app => app.routeName === 'contacts')
    if (appIndex3 !== -1) {
      Vue.set(state.config.apps[appIndex3], 'enabled', hasSim)
    }

    const appIndex4 = state.config.apps.findIndex(app => app.routeName === 'bank')
    if (appIndex4 !== -1) {
      Vue.set(state.config.apps[appIndex4], 'enabled', true)
    }

    const appIndex5 = state.config.apps.findIndex(app => app.routeName === 'tchat')
    if (appIndex5 !== -1) {
      Vue.set(state.config.apps[appIndex5], 'enabled', hasSim)
    }

    const appIndex6 = state.config.apps.findIndex(app => app.routeName === 'calculator')
    if (appIndex6 !== -1) {
      Vue.set(state.config.apps[appIndex6], 'enabled', true)
    }
