		...
		if isPedDrivingAVehicle() then
			...
			if pedInSameVehicleLast then
				...
				if healthEngineCurrent ~= 1000.0 or healthBodyCurrent ~= 1000.0 or healthPetrolTankCurrent ~= 1000.0 then
					...
					-- Prevent Explosions
					if not cfg.compatibilityMode and healthPetrolTankCurrent < 750 then
				...
				if newFCollisionDamageMultiplier <= 0 then newFCollisionDamageMultiplier = 1.0 end
				...
				if newFEngineDamageMult <= 0 then newFEngineDamageMult = 1.0 end
			...
			if pedInSameVehicleLast then
				print('We just got out of the vehicle')
